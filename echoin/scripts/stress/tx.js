...
function sendRawTx(stats, transaction, callback) {
  ...
  web3.ec.sendRawTx(rawTx, (err, resp) => {
    ...
    if (err) {
      ...
      let amount = Number(web3.fromWei(txInner.data.amount, "EC"))
...
function getShares(acc, valPubKey) {
  let shares = 0
  let result = web3.ec.stake.delegator.query(acc)
  ...
  return Number(web3.fromWei(shares, "EC"))
}
