...
let geetest = new Geetest({
  geetest_id: process.env['ECHOIN_FAUCET_GEETEST_ID'],
  geetest_key: process.env['ECHOIN_FAUCET_GEETEST_KEY']
})

const BaseAddr = '0x7eff122b94897ea5b0e2a9abf47b86337fafebdc'
const BasePwd = process.env['ECHOIN_FAUCET_COINBASE_PWD']
...
if (typeof web3 !== 'undefined') {
  ...
  web3 = new Web3(new Web3.providers.HttpProvider('http://' + process.env['ECHOIN_FAUCET_RPC_HOST'] + ':' + process.env['ECHOIN_FAUCET_RPC_PORT']));
}
...
app.post('/nodes', (req, res) => {
  request.get(
    {
      url: process.env['ECHOIN_FAUCET_TM_URL'] + '/net_info'
    },
    (error, response, body) => {
      if (error) {
        if (error.errno) {
  ...
  request.post(
    {
      ...
      form: {
        secret: process.env['ECHOIN_FAUCET_GOOGLE_RECAPTCHA_SECRET'],
  ...
  web3.eth.sendTransaction(
    ...
    function(err, ecHash) {
      ...
      testTokenInstance.transfer.sendTransaction(to, 1e21, {from: BaseAddr}, function(err, testHash) {
        web3.personal.lockAccount(BaseAddr)
        res.json({"ecHash": ecHash, "testHash": testHash})
      })
    }
  )
}
