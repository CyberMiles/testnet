...
do
  ...
  # echoin node init
  ECHOIN_NODE="docker run --rm -v $dir:/echoin blockservice/echoin:latest node"
  VM_GENESIS=""
  if [[ "$CHAIN_ID" == "stress" ]]; then
    cp ../../scripts/stress/vm-genesis.json .
    VM_GENESIS="--vm-genesis /echoin/vm-genesis.json"
  elif [[ "$CHAIN_ID" == "staging" ]]; then
    curl https://raw.githubusercontent.com/blockservice/testnet/master/echoin/init-staging/vm-genesis.json > ./vm-genesis.json
    VM_GENESIS="--vm-genesis /echoin/vm-genesis.json"
  elif [[ "$CHAIN_ID" == "mainnet" ]]; then
    curl https://raw.githubusercontent.com/blockservice/testnet/master/echoin/init-mainnet/vm-genesis.json > ./vm-genesis.json
    VM_GENESIS="--vm-genesis /echoin/vm-genesis.json"
  fi
  `$ECHOIN_NODE init --home /echoin --env $CHAIN_ID $VM_GENESIS`

  if [[ $i -le $INST_COUNT ]]; then
    SEEDS+=("$(${ECHOIN_NODE} show_node_id --home /echoin)@node-$i:$TP2PPORT")
  fi
  ...
  if [[ "$CHAIN_ID" == "staging" ]]; then
    sed -i.bak "s/rpcapi = .*$/rpcapi = \"ec,eth,net,web3\"/" ./config/config.toml
...
DEST=$HOME/volumes/testnet/echoin/nodes
