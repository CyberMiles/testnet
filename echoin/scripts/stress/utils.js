  ...
  return new Promise((resolve, reject) => {
    async.eachLimit(
      ...
      (account, cb) => {
        ...
        web3.ec.getTransactionCount(acc, (err, res) => {
...
exports.checkInterval = () => {
  logger.info(`start at height ${web3.ec.blockNumber}...`)

  let processed = 0
  let lastBlock = -1

  let interval = setInterval(() => {
    let currentBlock = web3.ec.blockNumber
    if (lastBlock == currentBlock) return

    let block = web3.ec.getEcBlock(currentBlock)
